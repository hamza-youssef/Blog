...
export const downvoteBlogPosts = (id) => async (dispatch) => {
  try {
    const { data } = await api.DownvoteSingleBlogPost(id);
    console.log(data)
...
export const getSingleBlog = (id) =>  (dispatch) => {
  try {
    // const { data } =  api.getSinglePost(id);
    // console.log("This is data : ",data)
    console.log("first",id)
    dispatch({ type: "GET_SINGLE_BLOG_POST", payload: id });
  } catch (error) {
    console.log(error.message);
  }
};